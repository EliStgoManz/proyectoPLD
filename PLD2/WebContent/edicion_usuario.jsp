<%@page import="datosRatos.DatosRepresentantePLD"%>
<%@page import="entidad.RepresentantePLD"%>
...
<body onload="startTimer(15)">
	...
	<h5 class="title title-contrata-ahora white-text">
		Sistema de Prevención de Lavado de Dinero-prueba jsp
		</h1>
		<div class="container">
			<div class="main row">
				<article class="col-xs-12 col-sm-8 col-md-9 col-lg-9">
					<h2 class="text-muted">Edición de usuarios</h2>
				</article>
			...
			<div class="row">
				<div class="col-md-9 table-responsive">
					<!-- SERVLET ControlCatalogoUsuario -->
					<form action="UsuariosSistema" method="post"
						...
						<br> <input id="perfilId" name="perfilId" type="hidden"
							...
							value="<c:out value="${supervisor}" />">
							<input
							id="id_sisegId" name="id_sisegId" type="hidden"
							value="<c:out value="${id_siseg}" />">
							 <input
						...
						<table border="0">
							...
							<tr>
								<td colspan="5" id="celdaPerfil" name="celdaPerfil"><label
									for="perfil">Perfil</label> <select class="browser-default"
									name="perfil" id="perfil" onchange="getPerfil()">
										...
										<% 
                                                                    ...
                                                                    if ( perfiles != null) {
                                                                        for ( int i = 0; i < perfiles.length; i ++ ){
                                                                            out.println("<option value=\"" + perfiles[i].getPerfilId()+ "\">" + perfiles[i].getDescripcion() + "</option>");
                                                                        }  
                                                                    }                                                                
                                                    %>
								</select></td>
								<td colspan="5" id="celdaRepre" name="celdaRepre"><label
									for="id_siseg">Representante Legal</label> <select class="browser-default"
									name="id_siseg" id="id_siseg" onchange="getRepresentante()">
										<option value="">* Seleccionar representante</option>

										<% 
                                                                   
                                                                    RepresentantePLD[] perfile = new DatosRepresentantePLD().getList("");
                                                                    if ( perfile != null) {
                                                                        for ( int i = 0; i < perfile.length; i ++ ){
                                                                            out.println("<option value=\"" + perfile[i].getId_siseg()+ "\">" + perfile[i].getNombre() + "</option>");
                                                                        }  
                                                                    }                                                                
                                                    %>
								</select></td>
							...
							<tr>
								...
								<td colspan="5" id="celdaSupervisor" name="celdaSupervisor">
									...
									class="browser-default" name="supervisor" id="supervisor">
										...
										<% 
                                                                    ...
                                                                    //Agregando los supervisores
                                                                    UsuarioSistema[] supervisores = new DatosUsuarioRatos().getSupervisor();
                                                                    if ( supervisores != null) {
                                                                        for ( int i = 0; i < supervisores.length; i ++ ){
                                                                            out.println("<option value=\"" + supervisores[i].getUsuario()+ "\">" + supervisores[i].getApellido_y_nombres() + "</option>");
                                                                        }  
                                                                    }
		...
		<script>
            ...
            txtRepre = document.getElementById("id_siseg").value;
            if(txtRepre == null || txtRepre.length == 0 || /^\s+$/.test(txtRepre)){
                alert('Debe seleccionar un Representante de usuario');
                return false;
            }
      ...
      if ( n  < 0 && m < 0){
          alert('El correo electrónico debe pertenecer a Efectivale o Fleetcor');
		...
		<script>
            ...
            $(document).ready(function(){ //lo que pasa cuándo se ha cargaro la lágina
                ...
                if  ( esEdicion == 1 ){ //la carga de la página proviene de una edición del usuario y no de un nuevo usuario
                    ...
                    txtRepresentante = document.getElementById('id_sisegId').value;
                        if(txtRepresentante != null || txtRepresentante.length > 0){                          
                            
                            var selector3 = document.getElementById('id_siseg');
                            
                            for ( var i = 0; i<selector3.length; i++){
                                if ( txtRepresentante  == selector3.options[i].value ) {
                                    selector3.selectedIndex = i;   
                                    document.getElementById('id_siseg').value = txtRepresentante;
			...
			//if ( txtPerfilUsuarioSistema == PERFIL_ADMINISTRADOR_TELEMARKETING && txtPerfil == PERFIL_EJECUTIVO_DE_VENTAS ) {
										if (txtPerfilUsuarioSistema == PERFIL_ADMINISTRADOR_TELEMARKETING) {
											...
											documente.getElementById("id_siseg").readOnly = true;
...
// 									} // if si existe el supervisor
			...
			function getRepresentante() {
				//alert('cambiando perfil');
				txtPerfilId = document.getElementById("id_siseg").value;
				document.getElementById("id_sisegId").value = txtPerfilId;
			}
		</script>
</body>
</html>