...
//                                                                         }
		...
		<script>
			...
			function validarFormularioGobierno() {
				...
				txtRFC = document.getElementById('rfcCliente').value;
...
// 				txtRFC2 = document.getElementById('fRFC').value;
...
// 				}
				...
				txtActividad = document.getElementById('actividadObjeto').value;
				if (txtActividad == null || txtActividad.length == 0
						|| /^\s+$/.test(txtActividad) || txtActividad == "1000000") {
					alert('Debe seleccionar la Actividad u Objeto Social');
					document.getElementById('actividadObjeto').focus();
					return false;
				}
	...
	function validarRFC(){
		...
		if(ruta != "0"){
			txtbRFC.trim();
		if(txtbRFC != null && txtbRFC != "" && !txtbRFC.IsEmpty()){
		$.ajax({
			async:true,
			crossDomain : true,
			type : "GET",
			...
			success : function(data) {
			  obj = JSON.parse(data);
				...
				if(obj.respuesta=="RFC Valido"){
					if(txtRFC != null && txtRFC != "" && !txtbRFC.IsEmpty()){
					$.ajax({
						async:true,
						crossDomain : true,
						type : "GET",
